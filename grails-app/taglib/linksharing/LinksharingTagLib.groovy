...
//            Resource resource =Resource.get(attrs.id)
            ...
            if ((ReadingItem.findByResourceAndIsRead(resource1, false)) || ReadingItem.findByResourceAndIsRead(resource2, false)) {
                //out<< "mark as unread"
    ...
    def ToggleResource = { attrs, body ->
        ...
        if (resource1) {
           // out << "view url"
            out << "<a href='/LinkResource/viewUrl/${attrs.id} absolute:true'> view url </a>"
        }

        if (resource2) {
            out << "<a href='/DocumentResource/download/${attrs.id}'> download </a>"
        }
